...
func init() {
	flag.Usage = func() {
		_, _ = fmt.Fprint(os.Stderr, "The s3analyser is a tool that shows information about an AWS account s3 buckets.\n"+
...
func main() {
	...
	if err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "Could not get objects from s3 %v", err)
		os.Exit(1)
	}
	...
	if err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "Unable to analyse s3. "+
